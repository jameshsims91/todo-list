...
let activeAnytimeId = null;

export function initAnytime() {
  console.log("🔍 initAnytime() event delegate listener registered!");
  ...
  if (anytimeBtn && !anytimeBtn.querySelector('.side-icon')) {
  ...
  const renderAnytimeDashboard = () => {
    console.log("🚀 renderAnytimeDashboard() engine firing into #content canvas!");
    const savedTasks = JSON.parse(localStorage.getItem('app_tasks')) || [];
    
    // Filter records tracking exclusively under the 'anytime' classification
    const anytimeTasksRaw = savedTasks.filter(task => task.category === 'anytime');
    const anytimeTasks = sortTasksByDueDate(anytimeTasksRaw);
    ...
    if (anytimeBtn) anytimeBtn.classList.add('active');

    // Inject Split-Pane Layout Blueprint into <div id="content">
    contentArea.innerHTML = `
      <div class="view-header">
        <h1>⏳ Anytime Dashboard</h1>
      </div>
      <div class="anytime-layout-grid">
        <!-- Left Column: Master Explorer List -->
        <div class="anytime-master-panel">
          <h3>Flexible Tasks</h3>
          <ul id="anytime-master-list">
            ${anytimeTasks.map(task => `
              <li class="anytime-list-card ${activeAnytimeId === task.id ? 'selected' : ''}" data-anytime-id="${task.id}">
                <div class="priority-dot-indicator status-${task.priority}"></div>
                <div class="anytime-card-header">
                  <strong>${task.name}</strong>
                  <span class="todo-count-badge">📝 ${task.todos ? task.todos.length : 0} items</span>
                </div>
              </li>
            `).join('')}
            ${anytimeTasks.length === 0 ? '<p class="empty-state-text">No active anytime tasks found. Click "Add Task" to get started.</p>' : ''}
          </ul>
        </div>

        <!-- Right Column: Detail Panel Canvas -->
        <div id="anytime-detail-panel" class="anytime-detail-panel">
          <div class="panel-placeholder">
            <p>Select an anytime task item from the explorer list to manage details, view nested items, or append sub-todos.</p>
          </div>
        </div>
      </div>
    `;

    const masterList = document.getElementById('anytime-master-list');
    if (masterList) {
      masterList.addEventListener('click', (e) => {
        const card = e.target.closest('.anytime-list-card');
        if (!card) return;

        masterList.querySelectorAll('.anytime-list-card').forEach(c => c.classList.remove('selected'));
        card.classList.add('selected');

        const taskId = card.getAttribute('data-anytime-id');
        activeAnytimeId = taskId; 
        
        const targetedTask = anytimeTasks.find(t => t.id === taskId);
        if (targetedTask) {
          renderAnytimeDetails(targetedTask, savedTasks, () => renderAnytimeDashboard(contentArea, anytimeBtn));
        }
      });
    ...
    if (activeAnytimeId) {
      const activeTask = anytimeTasks.find(t => t.id === activeAnytimeId);
      if (activeTask) {
        renderAnytimeDetails(activeTask, savedTasks, () => renderAnytimeDashboard(contentArea, anytimeBtn));
      } else {
        activeAnytimeId = null; 
      }
    }
  };
  ...
  document.addEventListener('click', (e) => {
    const anytimeBtn = e.target.closest('#btn-anytime');
    if (!anytimeBtn) return;

    console.log("🎯 Anytime sidebar button found and intercepted at runtime!");
    const contentArea = document.getElementById('content');
    ...
    // Format the sidebar button icon natively if needed
    if (!anytimeBtn.querySelector('.side-icon')) {
      const text = anytimeBtn.textContent.trim();
      anytimeBtn.innerHTML = `
        <span class="side-icon">⏳</span>
        <span class="side-text">${text}</span>
    ...
    activeAnytimeId = null; 
    renderAnytimeDashboard(contentArea, anytimeBtn); 
  });
}

// Detail Panel Window Sub-Renderer
function renderAnytimeDetails(task, allTasks, refreshParentDashboard) {
  const detailPanel = document.getElementById('anytime-detail-panel');
  if (!detailPanel) return;

  const displayDate = task.date ? format(parseISO(task.date), 'MMMM d, yyyy') : 'No due date set';

  detailPanel.innerHTML = `
    <div id="anytime-view-mode">
      ...
      <div class="detail-header-row" style="margin-top: 1rem;">
        ...
        <div class="detail-header-actions-cluster">
          ...
          ` : ''}
          <button type="button" id="duplicate-anytime-btn" class="duplicate-task-btn" title="Duplicate task pattern">👥 Duplicate</button>
          <button id="edit-anytime-meta-btn" class="edit-task-btn">✏️ Edit Details</button>
        </div>
      </div>
      
      <div class="anytime-meta-strip">
      ...
      ${task.note ? `<div class="anytime-notes-box" style="margin-top: 1.25rem;"><h4>Notes:</h4><p>${task.note}</p></div>` : ''}

      <h3 style="margin-top: 1.5rem;">Anytime Checklist Items</h3>
    ...
    <div id="anytime-edit-mode" style="display: none;">
      <div class="view-header form-header-split">
        <h3>✏️ Modify Anytime Properties</h3>
        <button type="button" id="cancel-p-edit-btn" class="cancel-icon-btn" title="Cancel changes">✕</button>
      </div>
      <form id="anytime-property-edit-form" class="property-edit-form">
        ...
        <div class="custom-cal-trigger-wrapper">
          <button type="button" id="edit-p-date-trigger" class="custom-date-trigger-btn" data-raw-date="${task.date || ''}">
            📅 ${task.date ? format(parseISO(task.date), 'MMM d, yyyy') : 'Set a target deadline...'}
          </button>
        </div>
        ...
        <div class="edit-mode-actions">
          ...
          <button type="button" id="delete-anytime-btn" class="delete-anytime-btn">🗑️ Delete Task</button>
        </div>
      </form>
    </div>
  `;
...
function setupDetailPanelListeners(task, allTasks, refreshParentDashboard) {
  const viewModeDiv = document.getElementById('anytime-view-mode');
  const editModeDiv = document.getElementById('anytime-edit-mode');
  
  const editMetaBtn = document.getElementById('edit-anytime-meta-btn');
  const cancelEditBtn = document.getElementById('cancel-p-edit-btn');
  const deleteAnytimeBtn = document.getElementById('delete-anytime-btn');

  const metaEditForm = document.getElementById('anytime-property-edit-form');
  const duplicateBtn = document.getElementById('duplicate-anytime-btn');
  ...
  if (clearFinishedBtn) {
    clearFinishedBtn.addEventListener('click', () => {
      ...
      renderAnytimeDetails(task, allTasks, refreshParentDashboard); // Smooth inline screen refresh
    });
  }
  ...
  if (deleteAnytimeBtn) {
    deleteAnytimeBtn.addEventListener('click', () => {
      if (confirm(`Are you sure you want to permanently delete "${task.name}"?`)) {
        ...
        const anytimeBtn = document.querySelector('#btn-anytime');
        if (anytimeBtn) anytimeBtn.click();
      }
    });
  }
  ...
  if (metaEditForm) {
    metaEditForm.addEventListener('submit', (e) => {
      e.preventDefault();
      task.name = document.getElementById('edit-a-name').value.trim();
      task.priority = document.getElementById('edit-a-priority').value;
      task.date = document.getElementById('edit-a-date-trigger').getAttribute('data-raw-date');
      task.note = document.getElementById('edit-a-note').value.trim();
  ...
  if (duplicateBtn) {
    duplicateBtn.addEventListener('click', () => {
      // Deep clone object schema and map a unique identity hash and title flag
      const duplicatedTask = {
        ...task,
        id: Date.now().toString(),
        name: `${task.name} (Copy)`,
        // Clone internal todo arrays by value to isolate completions states
  ...
  // B. Persistent Checked LocalStorage Syncing Toggles 💾
  if (checklist) {
    checklist.addEventListener('change', (e) => {
      if (e.target.classList.contains('todo-complete-checkbox')) {
        ...
        // Mutate completed sub-item boolean state inline
        ...
        if (task.category === 'anytime') renderAnytimeDetails(task, allTasks, refreshParentDashboard);
      }
    });
  }
  ...
  if (inlineTodoForm) {
    inlineTodoForm.addEventListener('submit', (e) => {
      ...
      if (todoValue) {
        ...
        renderAnytimeDetails(task, allTasks, refreshParentDashboard);
      }
    });
  }
